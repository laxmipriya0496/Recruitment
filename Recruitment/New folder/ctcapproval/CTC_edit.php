...
$stmt = $con->prepare("SELECT cfd.first_name,zdm.dept_name,cfd.position,rd.current_ctc,rd.exp_ctc FROM `md_commands` md join candidate_form_details cfd on md.candidate_id=cfd.id join z_department_master zdm on zdm.id=cfd.department join recruiter_details rd on cfd.id=rd.candidate_id where md.status=7 and cfd.id='$id'"); 
$stmt->execute(); 
$row = $stmt->fetch();
