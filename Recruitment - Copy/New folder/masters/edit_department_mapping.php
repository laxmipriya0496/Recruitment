...
require '../../connect.php';
 $id=$_REQUEST['id'];
$stmt = $con->prepare("SELECT *,d.id as dmid,c.companyname as cname,z.dept_name as dname,u.user_name as uname FROM `department_mapping` d join `z_department_master` z on d.department_id=z.id join z_user_master u on u.user_id=d.department_head join company_master c on c.id=d.department_id where d.id='$id'");
$stmt->execute(); 
$row = $stmt->fetch();
$sta=$row['status'];
...
<form role="form" name="" action="recruitment/masters/update_depmapping.php" method="post" enctype="multipart/type">
...
<?php
$sql=$con->query("SELECT * FROM z_user_master where user_group_code='ROLE-007'");
      $i=1;
      while($cmp = $sql->fetch(PDO::FETCH_ASSOC))
      {
		  ?>
		  <option value="<?php echo $cmp['user_id'];?>"><?php echo $cmp['user_name'];?></option>
		  <?php
	  }
		  ?>
		  </select>
