    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
		<?php if($userrole=='ROLE-004' && $userrole=='ROLE-005' && $userrole=='ROLE-008')	{?>
...
<th>Status</th>
      ...
      <tbody>
      <?php
      $emp_sql=$con->query("SELECT * FROM candidate_form_details where status=11 or status=0 or status=1 or status=3 or status=4 or status=5 or status=6 or status=13 or status=14 or status=15 or status=7 or status=8 or status=9 or status=10");
      $i=1;
	 ...
	 <?php
		  ...
		  echo '<span style="color:green;text-align:center;"><b>Selected For Second Level technical</b></span>';
	...
	<table id="example1" class="table table-bordered">
      ...
      <tr>
		...
		<span style="color:orange;text-align:center;"><b>PENDING</b></span>
		  ...
		  ?>
		  <span style="color:green;text-align:center;"><b>Selected for Second Level technical</b></span>
		  <?php
	  ...
	  <td><?php echo $i; ?></td>
		...
		<span style="color:orange;text-align:center;"><b>PENDING</b></span>
		  ...
		  ?>
		   ...
		   <td><?php if($emp_res['status'] == 4 || $emp_res['status'] ==10){
			  ?>
	...
	<table id="example1" class="table table-bordered">
      ...
      <tbody>
      <?php
      $emp_sql=$con->query("SELECT * FROM candidate_form_details where  status=4 or status=13 or status=14 or status=15 or status=7 or status=9 or status=10 ");
      $i=1;
      while($emp_res = $emp_sql->fetch(PDO::FETCH_ASSOC))
      {
      $emp_id = $emp_res['id'] ;
	...
	<table id="example1" class="table table-bordered">
      ...
      <tbody>
      <?php
      $emp_sql=$con->query("SELECT * FROM candidate_form_details where  status=11 or status=0 or status=1 or status=3 or status=4 or status=5 or status=6 or status=13 or status=14 or status=15 or status=7 or status=8 or status=9 or status=10 or status=16");
      $i=1;
      while($emp_res = $emp_sql->fetch(PDO::FETCH_ASSOC))
      {
      $emp_id = $emp_res['id'] ;
	 ...
	 <?php
	  ...
	  <?php }
		  ...
		  ?>
		   <span style="color:green;text-align:center;"><b>Document Pending</b></span>
		   <?php
		  }
		  else if($emp_res['status'] == 17){
		  ?>
		   <span style="color:green;text-align:center;"><b>Document Submitted</b></span>
		   <?php
		  }
		   else if($emp_res['status'] == 18){
		  ?>
		   <span style="color:green;text-align:center;"><b>Appointment Sended</b></span>
		   <?php
		  }
	  ?>
     </td>
		   <td><?php if($emp_res['status'] == 7 ){
			  ?>
...
function technical_edit(v){
 ...
 function technical1_edit(v){
	...
	$.ajax({
	type:"POST",
	url:"Recruitment/interview_feedback/final_technical_edit.php?id="+v,
	success:function(data)
	{
		$(".content").html(data);
	}
  ...
  function technical_view(v){
   ...
   function technical1_view(v){
	$.ajax({
	type:"POST",
	url:"Recruitment/interview_feedback/final_technical_view.php?id="+v,
	success:function(data)
	{
		$(".content").html(data);
	}
	...
	$.ajax({
	type:"POST",
	url:"Recruitment/interview_feedback/finaltechnical_insert.php?id="+v,
	success:function(data)
	{
		$(".content").html(data);
	}
...
function admin_view(v)
{
	$.ajax({
	type:"POST",
	url:"Recruitment/interview_feedback/admin_view.php?id="+v,
	success:function(data)
	{
		$(".content").html(data);
	}
...
function mail_Send(id)
{
	$.ajax({
	type:"POST",
	url:"Recruitment/interview_feedback/mail_send.php?id="+id,
	success:function(data)
	{
		if(data==0)
		{
